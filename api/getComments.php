...
if (isset($_GET["highlight"])) {
    $highlight = sprintf("SELECT * FROM `comments`
                          WHERE `comID`=%s
                          AND `%s`=%s
                          UNION ALL ", intval($_GET["highlight"]), $type, $fuckupID);
}

// How many comments should be fetched and the offset (page)
$dbSlice = clamp(intval($_GET["fetchAmount"]), 2, 15) * intval($_GET["page"]);
$query = sprintf($highlight . "SELECT * FROM `comments`
                               WHERE `comID`<=%d AND %s=%d
                               ORDER BY `comID` DESC
                               LIMIT %d 
                               OFFSET %s",
    $_GET["startID"], $type, $fuckupID, clamp(intval($_GET["fetchAmount"]), 2, 15), $dbSlice);
