...
function parseElapsed(secs: number) {
	...
	else if (secs < 1892160000) return Math.round(secs / 604800) + 'w'; //w - weeks
	else return Math.round(secs / 1892160000) + 'y'; //y - years
}

const getGradient = () =>
  `linear-gradient(90deg, ${listColor.value.darken(2)}, ${listColor.value.darken()}, ${listColor.value.brighten()})`
...
<template>
  ...
  >
    <section v-if="rate_ratio" class="flex flex-col items-center text-xs">
      <img src="../../images/genericRate.svg" alt="" class="w-3.5" />{{ rate_ratio }}
    </section>
    ...
    <section class="flex flex-col gap-1 items-center">
      ...
      <div v-if="timestamp" class="flex gap-1 text-xs hover:cursor-help" :title="`${uploadDate.toLocaleDateString()} ${uploadDate.toLocaleTimeString()}`">
        <img src="../../images/time.svg" alt="" class="w-4" />{{ parseElapsed(Date.now() / 1000 - timestamp) }}
      </div>
    </section>
