...
import type { FavoritedLevel, LevelTag } from '@/interfaces'
...
<template>
    <section class="max-w-[95vw] w-[70rem] mx-auto relative px-2 py-3 text-white rounded-lg backdrop-blur-sm" :style="{background: chroma(color).css()}">
        <div class="relative w-8">
            <img class="absolute left-1/2 z-10 w-8 t-1/2" :src="`/src/images/faces/${difficulty[0]}.webp`" alt="">
            <img class="absolute top-1/2 left-1/2 w-24" :src="`/src/images/${['', 'star', 'faces/featured','faces/epic'][difficulty[1]]}.webp`" alt="">
        </div>
