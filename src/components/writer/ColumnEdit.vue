...
const emit = defineEmits<{
    (e: "columnCommand", command: number): void
    (e: "close"): void
}>()
    ...
    <div class="m-2">
        <div class="flex flex-col gap-3 justify-center mb-5 text-xl text-center text-white">
            <div v-for="(setting, index) in [$t('other.add'), $t('other.duplicate')]">
                ...
                <div>
                    <button :disabled="postData.containers[selectedNest[0]].settings.components.length > 9" @click="emit('columnCommand', index*2)" class="box-border p-1 w-9 bg-black bg-opacity-40 rounded-md disabled:opacity-40 hover:bg-opacity-60 aspect-square">
                        <img src="@/images/moveUp.svg" class="mx-auto w-5 -rotate-90 button" alt="">
                    </button>
                    <button :disabled="postData.containers[selectedNest[0]].settings.components.length > 9" @click="emit('columnCommand', index*2+1)" class="box-border p-1 ml-2 w-9 bg-black bg-opacity-40 rounded-md disabled:opacity-40 hover:bg-opacity-60 aspect-square">
                        <img src="@/images/moveUp.svg" class="mx-auto w-5 rotate-90 button" alt="">
                    </button>
                </div>
            </div>
            ...
            <button @click="emit('columnCommand', 7); emit('close')" class="flex gap-1 items-center p-1 text-base font-bold text-black bg-red-400 rounded-md">
        ...
        <div class="flex gap-1 justify-between">
            ...
            <button @click="emit('columnCommand', 10); emit('close')" class="flex gap-1 items-center p-1 ml-6 text-xl font-bold text-black bg-red-400 rounded-md">
