...
const levelRatings = computed(() => reviewData.value.levels[selectedLevel.value!].ratings)
...
<template>
    ...
    <section :class="{'pt-10': settings.level == -1, '!max-w-[40rem]': settings.show != -1}" class="relative px-3 py-1 max-w-[40rem]  font-[poppins] bg-black bg-opacity-40 rounded-md overflow-x-hidden" v-else>
        <div v-if="settings.level == -1" class="flex absolute top-1 left-1/2 gap-4 justify-between w-max text-xl text-center border-b-2 -translate-x-1/2 bg-lof-200 border-lof-400">
            <button :class="{'to-15%': settings.show == -1}" class="text-right text-transparent whitespace-nowrap bg-clip-text bg-gradient-to-l from-white to-transparent opacity-60 min-w-64" @click="selectedLevel = Math.max(0, selectedLevel - 1)">{{ reviewData.levels?.[selectedLevel - 1]?.levelName }}</button>
            <button class="my-1 whitespace-nowrap rounded-md bg-lof-300 min-w-36">{{ reviewData.levels[selectedLevel].levelName }}</button>
            <button :class="{'to-15%': settings.show == -1}" class="text-left text-transparent whitespace-nowrap bg-clip-text bg-gradient-to-r from-white to-transparent opacity-60 min-w-64" @click="selectedLevel = Math.min(selectedLevel + 1, reviewData.levels.length - 1)">{{ reviewData.levels?.[selectedLevel + 1]?.levelName ?? '' }}</button>
        </div>

        <div v-if="settings.show != -1">
            <div class="grid grid-cols-2 grid-rows-2 text-left">
        ...
        <div v-else>
            <div v-if="reviewData.rateTheme == 0" class="grid grid-rows-4 grid-flow-col gap-x-2">
                <div v-for="(rating, index) in levelRatings[0].concat(levelRatings[1])" class="grid grid-cols-2 grid-rows-2 w-full text-left max-w-52">
                    <span>{{ allRatings[index].name || $t('other.unnamesd') }}</span>
                    <span v-if="rating > -1" class="w-full text-xl font-bold text-right">{{ rating }}/10</span>
