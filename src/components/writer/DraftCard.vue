    ...
    <div @click="doOpen()" v-if="!hide" @keyup.enter="doOpen()" tabindex="0" class="w-full bg-black bg-opacity-40 rounded-md transition-transform focus:outline outline-2 draftCard outline-lof-400" :class="{'hover:translate-x-1 active:translate-x-2': !isOpen, 'border-l-4 border-lof-400 ': inUse}">
        <div class="flex flex-col justify-between p-2">
            ...
            <div v-if="inUse" class="grid grid-cols-3 gap-2 p-1">
                ...
                <button :disabled="justSaved" @click.stop="emit('action', DraftAction.Revert)" class="flex gap-2 items-center p-1 bg-black bg-opacity-40 rounded-md disabled:opacity-20"><img class="w-6" src="@/images/replay.svg">{{ $t('other.revert') }}</button>
            </div>
