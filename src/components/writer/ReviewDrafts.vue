...
import { getWordCount, reviewData } from "@/Reviews";
import type { LevelList, ReviewDraft, ReviewList } from "@/interfaces";
...
const emit = defineEmits<{
    (e: "saved", data: {backupID: number, lastSaved: number})
    (e: "preview", data: ReviewList)
    (e: "load", data: {data: ReviewList, id: number, saved: number})
}>()
...
const saveDraft = (saveAs: boolean) => {
    let now = Date.now()
    let backupID;
    let preview = parseReview()
    if (props.inUseID == 0 || saveAs) {
        let saveAsName = ""
        if (saveAs && props.inUseID != 0) {
            let i = 1
            funnySaveAsMessages.forEach(x => {
                if (drafts.value[props.inUseID].name.includes(x)) saveAsName = " " + funnySaveAsMessages[Math.min(i, funnySaveAsMessages.length-1)]
                i += 1
            })
        ...
        drafts.value[now] = {
            ...
            name: reviewData.value.reviewName || i18n.global.t('editor.unnamedReview') + saveAsName,
            createDate: now,
            saveDate: now,
        ...
        drafts.value[props.inUseID].reviewData = reviewData.value
        drafts.value[props.inUseID].saveDate = now
        drafts.value[props.inUseID].wordCount = getWordCount()
        drafts.value[props.inUseID].previewTitle = preview[0]
        drafts.value[props.inUseID].previewParagraph = preview[1]
        backupID = props.inUseID
    }
    localStorage.setItem("reviewDrafts", JSON.stringify(drafts.value))
    emit('saved', {backupID: backupID, lastSaved: now})
    justSaved.value = true
}

const editName = (newName: string, key: string) => {
    drafts.value[key].name = newName
    localStorage.setItem("reviewDrafts", JSON.stringify(drafts.value))
}

const removeDraft = (key: string) => {
    if (key == props.inUseID) emit('saved', {backupID: 0, lastSaved: 0})
...
<template>
    <div v-if="searchBoxShown" class="flex items-center px-1 my-1">
        ...
        <input type="text" v-model="query" :placeholder="$t('other.search')" class="px-2 py-1 ml-1 bg-black bg-opacity-40 rounded-md grow">
    </div>
    <div v-else class="grid grid-cols-3 gap-1 p-1">
        <button @click="saveDraft(false)" :disabled="justSaved" class="flex gap-2 p-1 pl-2 bg-black bg-opacity-40 rounded-md disabled:opacity-20"><img src="@/images/symbolicSave.svg" class="w-4" alt="">{{ justSaved ? $t('reviews.justSaved') : $t('other.save') }}</button>
        <button @click="saveDraft(true)" :disabled="inUseID == 0" class="flex gap-2 p-1 pl-2 bg-black bg-opacity-40 rounded-md disabled:opacity-20"><img src="@/images/copy.svg" class="w-4" alt="">{{ $t('other.duplicate') }}</button>
    ...
    <div
        ...
        <div v-if="!Object.keys(drafts).length" class="flex absolute top-1/2 left-1/2 flex-col gap-3 items-center w-3/4 text-center opacity-20 -translate-x-1/2 -translate-y-1/2">
            ...
            <h2 class="text-2xl">Tady jsou tvé nedopsané recenze!</h2>
            <p class="">Můžeš si tu své recenze ukládat a kdykoliv se k nim vrátit.</p>
            <p class="">Vše se ukládá lokálně. Zobrazí se tedy jen na tomto zařízení a když smažeš cookies, smažou se i tyto recenze. Ukládat můžeš i při výpadku internetu nebo serverů.</p>
            <p class="">Po nahrání se koncept smaže.</p>
        </div>

        <div v-if="!Object.keys(filteredDrafts).length" class="flex absolute top-1/2 left-1/2 flex-col gap-3 items-center w-3/4 text-center opacity-20 -translate-x-1/2 -translate-y-1/2">
        ...
        <div v-for="(draft, key) in filteredDrafts" @click="optionsOpen = optionsOpen == -1 || optionsOpen != key ? key : -1; editingName = -1" class="w-full bg-black bg-opacity-40 rounded-md transition-transform" :class="{'hover:translate-x-1 active:translate-x-2': optionsOpen != key, 'border-l-4 border-lof-400 ': inUseID == key}">
            <div class="flex flex-col justify-between p-2">
                <div class="flex flex-col">
                    <h2 v-if="editingName != key" @click.stop="editingName = key" class="flex gap-2 items-center text-xl leading-tight group">{{ draft.name }} <img src="@/images/edit.svg" class="w-2 opacity-0 transition-opacity group-hover:opacity-100"></h2>
                ...
                <div v-if="optionsOpen == key" class="grid grid-cols-3 gap-2 p-1">
                    <button @click.stop="removeDraft(key)" class="flex gap-2 p-1 bg-black bg-opacity-40 rounded-md"><img class="w-6" src="@/images/trash.svg">{{ $t('editor.remove') }}</button>
