...
<template>
    <Teleport :disabled="shown != 2" to="body">
        <form ref="containerSettings" @keydown.prevent.esc="forceHide()" @submit.prevent="" v-if="settingsShown" :style="{left: shown == 2 ? `${float[0]}px` : '', top: shown == 2 ? `${float[1]}px` : '-4px'}" class="flex w-max absolute text-white font-[poppins] gap-2 -right-1 z-20 flex-col p-2 text-base text-left rounded-md rounded-tr-none bg-greenGradient">
