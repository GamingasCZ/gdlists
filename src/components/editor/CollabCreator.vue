    ...
    <section class="flex flex-col gap-2 items-center pl-2 overflow-clip bg-black rounded-md odd:bg-opacity-40 even:bg-opacity-20">
        <main class="flex gap-2 items-center w-full">
            ...
            <section class="flex flex-col items-center">
                <h3>Role</h3>
                <button class="px-1 w-40 rounded-md button" :style="{backgroundColor: roleColor}" @click="emit('changeRole', pos)">
                    {{ levelList.levels[levelIndex].creator[1][role] || 'Bezejmenná' }}
                    <img src="@/images/edit.svg" class="box-border inline p-0.5 ml-auto w-4 bg-black bg-opacity-40 rounded-md" alt="">
                </button>
            </section>
            <section class="flex flex-col items-center">
                <h3>Část</h3>
