...
const moveTagPos = (from: number, to: number) => {
    if (to < 0 || to >= levelList.value.levels[props.cardIndex!].tags.length) return
    editingLink.value = false

    let currentTag = levelList.value.levels[props.cardIndex!].tags[props.tagIndex!]
...
<template>
    <div class="flex gap-2 items-center px-1 py-2 bg-black bg-opacity-20 rounded-md">
        <img src="@/images/showCommsL.svg" class="w-4 button" alt="" @click="moveTagPos(tagIndex!, tagIndex!-1)">
        <div class="flex flex-col gap-2 items-center">
            <div class="flex items-center">
                <LinkIcon class="w-5 button" @click="editingLink = !editingLink" :editing="editingLink" :modified="!(!tagLink?.length)"/>
